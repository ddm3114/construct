...
def get_optim(model, lr_head =0.01, lr_backbone = 0.001, weight_decay=0.001, train_backbone=False,optim = 'adam'):
    ...
    if optim == 'adam' or optim == 'Adam':
        ...
        if linear == 'fc':
            ...
            optimizer = torch.optim.Adam(model.model.parameters(), lr=lr_backbone, weight_decay=weight_decay)
            print(f"'[warning]{type(model).__name__}' object has no attribute 'head','fc'or 'classifier',all layer will be trained by backbone lr")
        ...
        if linear == 'fc':
            ...
            optimizer = torch.optim.SGD(model.model.parameters(), lr=lr_backbone, weight_decay=weight_decay)
            print(f"'[warning]{type(model).__name__}' object has no attribute 'head','fc'or 'classifier',all layer will be trained by backbone lr")
        ...
        if linear == 'fc':
            ...
            if train_backbone and lr_backbone:
                ...
                optimizer = torch.optim.AdamW([{'params': backbone_params, 'lr': lr_backbone},
                                            {'params': model.model.fc.parameters(), 'lr': lr_head}],
                                            weight_decay=weight_decay)
            else:
                optimizer = torch.optim.AdamW(model.model.fc.parameters(), lr=lr_head, weight_decay=weight_decay)
        elif linear == 'head':
            backbone_params = [param for name, param in model.model.named_parameters() if "head" not in name]
            if train_backbone and lr_backbone:
                ...
                optimizer = torch.optim.AdamW(model.model.head.parameters(), lr=lr_head, weight_decay=weight_decay)
        elif linear == 'classifier':
            backbone_params = [param for name, param in model.model.named_parameters() if "classifer" not in name]
            fc_params = [param for name, param in model.model.named_parameters() if "classifer" in name]
            if train_backbone and lr_backbone:
                ...
                optimizer = torch.optim.AdamW(model.model.classifer.parameters(), lr=lr_head, weight_decay=weight_decay)
        else:
            optimizer = torch.optim.AdamW(model.model.parameters(), lr=lr_backbone, weight_decay=weight_decay)
            print(f"'[warning]{type(model).__name__}' object has no attribute 'head','fc'or 'classifier',all layer will be trained by backbone lr") 
    else:
...
if __name__ == "__main__":
    ...
    model = get_model('Bert',pretrained=True,hidden_dim=4096,train_backbone=True,num_classes=10)
    lr_head = 0.01
    lr_backbone = 0.001
    weight_decay = 0.001
    train_backbone = True
