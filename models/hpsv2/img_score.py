...
def initialize_model(hps_version: str = "v2.0",cp: str = './weight',text_work = True):
    if not model_dict:
        ...
        elif hps_version == 'v2.1':
            cp = os.path.join(root_path,'HPS_v2.1_compressed.pt')
        else:
            raise ValueError('The version of HPS is not supported.')
        
        print(f"Loading checkpoint from {cp}")
        ...
        return model,tokenizer,preprocess_val,preprocess_train

def score(img_path: Union[list, str, Image.Image], prompt: str,model = None,tokenizer = None,preprocess_val = None) -> list:
    if model is None:
