...
def initialize(hps_version: str = "v2.0",text_work = True):
    from . import img_score as scr
    model,tokenizer,process_val,process_train = scr.initialize_model(hps_version,text_work = text_work)
    return model,tokenizer,process_val,process_train

def score(imgs_path: Union[list, str, Image.Image], prompt: str, model = None,tokenizer = None,preprocess_val=  None) -> list:
    ...
    if model is None or tokenizer is None:
        raise ValueError("The model or tokenizer is not initialized.")
    from . import img_score as scr
    res = scr.score(imgs_path, prompt,model = model,tokenizer = tokenizer,preprocess_val = preprocess_val)
    return res
